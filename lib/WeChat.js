  ...
  getMedia (media_id) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const { apiproxy } = this.config
        var getmedia_url = 'https://qyapi.weixin.qq.com/cgi-bin/media/get'
        if (apiproxy){
            getmedia_url = apiproxy + '/cgi-bin/media/get'
        }
        const result = await axios.get(getmedia_url, {
  ...
  uploadMedia (data) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        var uploadmedia_url = 'https://qyapi.weixin.qq.com/cgi-bin/media/upload'
        ...
        if (apiproxy){
            uploadmedia_url = apiproxy + '/cgi-bin/media/upload'
        }
        
        const result = await axios.post(uploadmedia_url, form, {
  ...
  pushMessage (sendData) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const { apiproxy } = this.config
        var messagesend_url = 'https://qyapi.weixin.qq.com/cgi-bin/message/send'
        if (apiproxy){
            messagesend_url = apiproxy + '/cgi-bin/message/send'
        }
        
        const result = await axios.post(messagesend_url, sendData, {
          params: { access_token }
        }).catch(err => {
